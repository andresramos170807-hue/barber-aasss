...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && req.url.startsWith('/api/probarfotos')) {
    ...
    // Palabras que suelen indicar una imagen
    const pareceFoto = (clave, valor) => {
      const k = (clave || '').toLowerCase();
      const v = String(valor || '').toLowerCase();
      const esClave = k.includes('foto') || k.includes('avatar') || k.includes('imagen') || k.includes('image') || k.includes('photo') || k.includes('picture') || k.includes('url');
      const esValor = v.includes('.jpg') || v.includes('.jpeg') || v.includes('.png') || v.includes('.webp') || v.startsWith('http');
      return esClave || esValor;
    };
    ...
    const analizar = (obj) => {
      ...
      Object.entries(obj).forEach(([k, val]) => {
        const tipo = Array.isArray(val) ? 'lista' : typeof val;
        campos[k] = tipo === 'object' ? '{objeto}' : (String(val).length > 60 ? String(val).substring(0, 60) + '...' : val);
        if (pareceFoto(k, val)) posiblesFotos[k] = val;
      });
      return { todos_los_campos: campos, POSIBLES_FOTOS: posiblesFotos };
    };

    const resultado = {};
    // Un empleado (viene en /agenda o /user). Probamos /user
    for (const ruta of ['/api/user/', '/api/empleado/', '/api/profesional/']) {
      try {
        const r = await koiboxFetch(`https://api.koibox.cloud${ruta}?limit=1`, headers);
        if (r.status === 200) {
          const j = await r.json();
          const primero = (j.results && j.results[0]) || null;
          resultado['EMPLEADO via ' + ruta] = primero ? analizar(primero) : 'sin registros';
          break;
        }
      } catch (e) {}
    }
    // Un producto
    for (const ruta of ['/api/producto/', '/api/productos/']) {
      try {
        const r = await koiboxFetch(`https://api.koibox.cloud${ruta}?limit=1`, headers);
        if (r.status === 200) {
          const j = await r.json();
          const primero = (j.results && j.results[0]) || null;
          resultado['PRODUCTO via ' + ruta] = primero ? analizar(primero) : 'sin registros';
          break;
        }
      } catch (e) {}
    }
