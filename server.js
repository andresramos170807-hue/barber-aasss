  ...
  while (nextUrl && pages < maxPages) {
    ...
    for (let intento = 0; intento < 3; intento++) {
      try {
        ...
        if (res.status === 429 || res.status >= 500) { await espera(800 * (intento + 1)); continue; }
        if (!res.ok) break;
        data = await res.json();
        break;
      } catch (e) {
...
server.listen(PORT, () => {
  ...
  (async () => {
    for (const l of ['Sevilla Este', 'Bormujos', 'Gines']) {
      try { await getDatosLocal(l, today); console.log('Cache lista:', l); }
      catch (e) { console.log('Fallo precalentando', l, e.message); }
    }
  })();
});
