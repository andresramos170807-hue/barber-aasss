...
async function getKoiboxData(local) {
  ...
  try {
    ...
    // Cogemos más páginas para tener datos más completos
    const [agenda, clientes, caja] = await Promise.all([
      fetch('https://api.koibox.cloud/api/agenda/?page_size=100', { headers }).then(r => r.json()),
      fetch('https://api.koibox.cloud/api/clientes/?page_size=100', { headers }).then(r => r.json()),
      fetch('https://api.koibox.cloud/api/caja/?page_size=100', { headers }).then(r => r.json()).catch(() => ({ error: 'no disponible' })),
    ]);

    return { agenda, clientes, caja };
  } catch (e) {
...
Analiza los datos y responde en español de forma específica y detallada. 
Usa tablas cuando sea útil para comparar datos.
Destaca en negrita los datos más importantes.
Si hay datos de barberos, clientes o caja, úsalos para dar respuestas precisas.
Máximo 200 palabras.
