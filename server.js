...
async function fetchAllPages(url, headers, maxPages = 60) {
  ...
  while (nextUrl && pages < maxPages) {
    let data = null;
    // hasta 3 intentos por pagina
    for (let intento = 0; intento < 3; intento++) {
      try {
        const res = await fetch(nextUrl, { headers });
        if (res.status === 429 || res.status >= 500) { console.log('KOIBOX status', res.status, '-> reintento'); await espera(1200 * (intento + 1)); continue; }
        if (!res.ok) { console.log('KOIBOX ERROR status', res.status, 'url', nextUrl); break; }
        data = await res.json();
        break;
      } catch (e) {
        await espera(500 * (intento + 1));
      }
    }
...
async function getDatosLocal(local, fechaEspecifica) {
  ...
  const [citasMesData, citasDiaData, ventasData, clientesData] = await Promise.all([
    ...
    fetchAllPages(`https://api.koibox.cloud/api/clientes/?limit=200`, headers, 2),
  ]);
  ...
  if (ventasData.completo && citasMesData.completo) cacheSet(cacheKey, datos);
  else console.log('AVISO: datos incompletos, no se cachean');
  return datos;
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && req.url.startsWith('/api/debug')) {
    ...
    for (const [local, key] of Object.entries(KOIBOX_KEYS)) {
      ...
      try {
        const r = await fetch(`https://api.koibox.cloud/api/ventas/?fecha__gte=${firstDay}&fecha__lte=${lastDay}&limit=1`, {
          headers: { 'X-Koibox-Key': key, 'Accept': 'application/json' }
        });
