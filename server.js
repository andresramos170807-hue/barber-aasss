...
async function getKoiboxData(local, question) {
  ...
  try {
    const [
      ...
      // Caja del mes
      fetchAllPages(
        `https://api.koibox.cloud/api/caja/?fecha__gte=${firstDay}&fecha__lte=${lastDay}&page_size=200`,
        headers, 10
      ).catch(() => []),
    ]);
    ...
    const barberosMes = citasMes.reduce((acc, cita) => {
      const nombre = cita.usuario_nombre || cita.empleado_nombre || cita.user_nombre || 'Sin asignar';
    ...
    const facturacionCaja = cajaEntradas.reduce((acc, entrada) => {
      return acc + (parseFloat(entrada.total) || parseFloat(entrada.importe) || 0);
    }, 0);
