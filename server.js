  ...
  if (req.method === 'GET' && req.url.startsWith('/api/debug2')) {
    ...
    for (const [local, key] of Object.entries(KOIBOX_KEYS)) {
      ...
      try {
        ...
        // Estructura de una venta (para poder analizar productos y empleados)
        const v0 = ventasData.results[0] || {};
        const camposVenta = Object.keys(v0);
        // Buscar el array de lineas dentro de la venta, se llame como se llame
        let nombreCampoLineas = null;
        let camposLinea = [];
        let ejemploLinea = null;
        for (const k of camposVenta) {
          if (Array.isArray(v0[k]) && v0[k].length && typeof v0[k][0] === 'object') {
            nombreCampoLineas = k;
            camposLinea = Object.keys(v0[k][0]);
            const l0 = v0[k][0];
            ejemploLinea = {};
            camposLinea.forEach(ck => {
              const val = l0[ck];
              ejemploLinea[ck] = (val && typeof val === 'object')
                ? '{' + Object.keys(val).slice(0, 8).join(', ') + '}'
                : val;
            });
            break;
          }
        ...
        salida[local] = {
          ...
          campos_de_una_venta: camposVenta,
          campo_con_las_lineas: nombreCampoLineas,
          campos_de_una_linea: camposLinea,
          ejemplo_de_linea: ejemploLinea,
        };
      } catch (e) {
