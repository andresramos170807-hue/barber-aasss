...
// Lee la pestana Empleados de un salon (si existe): Empleado | Sueldo | %SS | Horas | ...
async function leerEmpleadosSalon(local) {
  ...
  try {
    ...
    // Buscar la cabecera con "empleado" y "sueldo"
    let filaCab = -1;
    for (let i = 0; i < Math.min(filas.length, 6); i++) {
      const norm = filas[i].map(normaliza);
...
async function getCostes(mesPedido) {
  ...
  const [gastosNuevos, empleadosNuevos] = await Promise.all([
    ...
    Promise.all(locales.map(l => leerEmpleadosSalon(l).catch(() => null))),
  ]);
