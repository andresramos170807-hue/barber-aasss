...
async function descargarDatosLocal(local, fechaEspecifica, mesPedido, ck, key) {
  ...
  const nuevoEmp = () => ({ facturacion: 0, base: 0, fact_servicios: 0, base_servicios: 0, tickets: 0, productos: 0, servicios: 0, detalle_productos: {}, uso_interno: 0, detalle_uso_interno: {} });
  ...
  ventas.forEach(v => {
    ...
    lineasDeVenta(v).forEach(l => {
      ...
      if (a.tipo === 'Producto') {
        ...
        // Base sin IVA aproximada de la linea (misma proporcion que la venta)
        E.base_servicios += impLinea / 1.21;
      }
    });
  });
