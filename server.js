...
async function getCalidad() {
  ...
  ['Sevilla Este', 'Bormujos', 'Gines'].forEach(l => {
    out[l] = {
      ...
      calidad_personal_sobre: per[l] ? per[l].sobre : 10,
      fecha_personal: per[l] ? per[l].fecha : null,
      detalle_personal: per[l] ? per[l].detalle : null,
      observaciones_personal: per[l] ? per[l].observaciones : '',
...
async function analizarPersonal(local, datos) {
  const cal = datos.calidad_detalle;
  if (!cal || (!cal.detalle_personal && cal.calidad_personal === null)) {
    return { error: 'Aun no hay auditoria de personal para este local. Conecta el formulario o rellenalo.' };
  }
  ...
  const r = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'user',
        content: `Eres el encargado de RRHH de LeBarbier ${local}. Analiza la ultima auditoria de calidad del PERSONAL.

DATOS DE LA AUDITORIA:
- Nota global personal: ${cal.calidad_personal !== null ? cal.calidad_personal + ' sobre ' + (cal.calidad_personal_sobre || 10) : 'sin nota'}
- Evaluado: ${(cal.detalle_personal && cal.detalle_personal.evaluado) || 'no especificado'}
- Detalle apartado por apartado: ${JSON.stringify(cal.detalle_personal || {})}
- Observaciones: ${cal.observaciones_personal || 'ninguna'}

Tu trabajo:
1. Di claramente que apartados estan BIEN y cuales estan MAL, con su valor concreto.
2. Valora si la situacion es lo bastante seria como para PONER UN PARTE DE TRABAJO (aviso formal por escrito). Un parte se justifica cuando hay fallos graves o repetidos que afectan al cliente o al equipo (mala higiene, mala atencion, impuntualidad seria, incumplimiento de protocolo). Un fallo leve o puntual NO justifica un parte: basta con hablarlo.

Responde SOLO con un JSON valido, sin markdown, con esta forma:
{
  "nota_global": "texto corto con la nota",
  "hace_bien": ["apartado bien con su valor", "otro"],
  "hace_mal": ["apartado mal con su valor", "otro"],
  "merece_parte": true o false,
  "motivo_parte": "Si merece_parte es true, explica por que y sobre que apartados. Si es false, explica por que basta con hablarlo.",
  "texto_parte": "Si merece_parte es true, un borrador breve y profesional del parte listo para entregar. Si es false, cadena vacia.",
  "como_hablarlo": "Una recomendacion de como plantearselo al empleado de forma constructiva."
}

En español, concreto, justo y profesional. No exageres: si el trabajo es bueno, dilo y pon merece_parte en false.`
      }]
    })
  });
