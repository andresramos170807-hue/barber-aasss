        ...
        if (l.producto) {
          ...
          if (ejemplos.length < 4) {
            ...
            Object.keys(l).forEach(k => {
              const val = l[k];
              linePlano[k] = (val && typeof val === 'object') ? '{OBJ}' : val;
            });
            ejemplos.push({
              TODOS_LOS_CAMPOS_DE_LA_LINEA: linePlano,
              TODOS_LOS_CAMPOS_DEL_PRODUCTO: (typeof l.producto === 'object') ? l.producto : l.producto,
              venta_total: v.total,
              venta_base_imponible: v.base_imponible,
              empleado: nombreEmpleado(l.assigned_to) || empleadoDeVenta(v),
              lo_que_yo_interpreto: analizarLinea(l, empleadoDeVenta(v)),
            });
          }
        }
