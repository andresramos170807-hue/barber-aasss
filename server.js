...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && req.url.startsWith('/api/analisis-stock')) {
    try {
      ...
      let consejo = null;
      try {
        const resumen = lista.filter(x => x.existencias != null)
          .map(x => `${x.nombre}: ${x.existencias} en stock (min ${x.stock_minimo}), vendidos este mes ${x.vendidos_este_mes}`)
          .join('\n');
      ...
      } catch (e) { consejo = { error: 'No se pudo generar el consejo IA: ' + e.message }; }
