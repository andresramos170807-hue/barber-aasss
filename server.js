...
function empleadoDeVenta(v) {
  const a = v.assigned_to || v.empleado || v.user;
  if (a && typeof a === 'object') return a.first_name || a.username || a.nombre || 'Sin asignar';
  if (typeof a === 'string') return a;
  return v.empleado_nombre || 'Sin asignar';
}
...
// De una linea saca: nombre, si es producto o servicio, cantidad, importe y empleado
function analizarLinea(linea, ventaEmpleado) {
  let nombre = null, tipo = 'Otro';

  if (linea.producto) {
    tipo = 'Producto';
    nombre = typeof linea.producto === 'object'
      ? (linea.producto.nombre || linea.producto.name) : linea.producto;
  } else if (linea.servicio) {
    tipo = 'Servicio';
    nombre = typeof linea.servicio === 'object'
      ? (linea.servicio.nombre || linea.servicio.name) : linea.servicio;
  } else if (linea.bono || linea.pack) {
    tipo = 'Bono';
    const b = linea.bono || linea.pack;
    nombre = typeof b === 'object' ? (b.nombre || b.name) : b;
  }
  if (!nombre) nombre = linea.nombre || linea.concepto || linea.descripcion || 'Sin nombre';

  const cantidad = parseFloat(linea.cantidad) || parseFloat(linea.unidades) || 1;
  const importe = parseFloat(linea.total) || parseFloat(linea.precio) || parseFloat(linea.importe) || 0;

  let emp = ventaEmpleado;
  const la = linea.assigned_to || linea.empleado || linea.user;
  if (la && typeof la === 'object') emp = la.first_name || la.username || la.nombre || emp;

  return { nombre, tipo, cantidad, importe, empleado: emp };
}
...
async function getDatosLocal(local, fechaEspecifica) {
  ...
  let facturacion = 0;
  const porEmpleado = {};   // empleado -> { facturacion, tickets, productos, servicios, detalle }
  const porDia = {};
  const productosTop = {};
  const serviciosTop = {};
  ...
  ventas.forEach(v => {
    ...
    if (!porEmpleado[emp]) porEmpleado[emp] = { facturacion: 0, tickets: 0, productos: 0, servicios: 0, detalle_productos: {} };
    ...
    lineasDeVenta(v).forEach(l => {
      ...
      if (!porEmpleado[a.empleado]) porEmpleado[a.empleado] = { facturacion: 0, tickets: 0, productos: 0, servicios: 0, detalle_productos: {} };

      if (a.tipo === 'Producto') {
        porEmpleado[a.empleado].productos += a.cantidad;
        porEmpleado[a.empleado].detalle_productos[a.nombre] = (porEmpleado[a.empleado].detalle_productos[a.nombre] || 0) + a.cantidad;
        productosTop[a.nombre] = (productosTop[a.nombre] || 0) + a.cantidad;
      } else if (a.tipo === 'Servicio') {
        porEmpleado[a.empleado].servicios += a.cantidad;
  ...
  Object.keys(porDia).forEach(k => { porDia[k].facturacion = Math.round(porDia[k].facturacion * 100) / 100; });
...
- "tickets_cobrados" = ventas realmente cobradas este mes (esto es lo REAL).
- "citas_agendadas_mes" = citas en agenda, INCLUYE pendientes y anuladas. No es lo mismo que realizadas.
- "ranking_empleados" trae por cada empleado: facturacion, tickets, productos_vendidos y detalle_productos (nombre -> unidades).
- "okr" son los objetivos del mes. Compara el % logrado con "ritmo_esperado_pct" (% del mes ya transcurrido): si el % logrado es menor, van por detras.

INSTRUCCIONES:
- Responde en español, concreto y directo.
- Usa tablas markdown para rankings y desgloses.
- Si preguntan por productos de un empleado, usa "detalle_productos" y desglosa producto a producto con unidades.
