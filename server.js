...
async function getDatosLocal(local, fechaEspecifica) {
  ...
  const [citasMesData, citasDiaData, ventasData, clientesData] = await Promise.all([
    fetchAllPages(`https://api.koibox.cloud/api/agenda/?fecha__gte=${firstDay}&fecha__lte=${lastDay}&page_size=200`, headers, 120),
    fetchAllPages(`https://api.koibox.cloud/api/agenda/?fecha__gte=${fechaEspecifica}&fecha__lte=${fechaEspecifica}&page_size=200`, headers, 10),
    fetchAllPages(`https://api.koibox.cloud/api/ventas/?fecha__gte=${firstDay}&fecha__lte=${lastDay}&page_size=200`, headers, 120),
    fetchAllPages(`https://api.koibox.cloud/api/clientes/?page_size=200`, headers, 2),
  ]);

  const citasMes = citasMesData.results;
  const ventas = ventasData.results;

  console.log(`[${local}] ventas ${ventas.length}/${ventasData.count} | citas ${citasMes.length}/${citasMesData.count}`);
...
server.listen(PORT, () => {
  ...
  ['Sevilla Este', 'Bormujos', 'Gines'].forEach(l => {
    getDatosLocal(l, today).then(() => console.log('Cache lista:', l)).catch(() => {});
  });
});
