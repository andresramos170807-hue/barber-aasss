...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && req.url.startsWith('/api/probarfotos')) {
    ...
    // 2. EMPLEADOS: probar muchas rutas posibles
    const rutasEmpleado = [
      '/api/user/', '/api/users/', '/api/empleado/', '/api/empleados/',
      '/api/profesional/', '/api/profesionales/', '/api/staff/',
      '/api/trabajador/', '/api/agente/', '/api/personal/',
    ];
    resultado.EMPLEADOS = { rutas_probadas: {} };
    for (const ruta of rutasEmpleado) {
      try {
        const r = await koiboxFetch(`https://api.koibox.cloud${ruta}?limit=1`, headers);
        let info = { status: r.status };
        if (r.status === 200) {
          const txt = await r.text();
          try {
            const j = JSON.parse(txt);
            const primero = (j.results && j.results[0]) || null;
            info.EXISTE = true;
            info.total = j.count;
            if (primero) {
              info.campos = Object.keys(primero);
              // Buscar campos que parezcan foto
              const fotos = {};
              Object.entries(primero).forEach(([k, v]) => {
                const kk = k.toLowerCase();
                if (kk.includes('foto') || kk.includes('avatar') || kk.includes('imagen') || kk.includes('image') || kk.includes('photo')) fotos[k] = v;
              });
              info.POSIBLES_FOTOS = fotos;
            }
          } catch (e) { info.EXISTE = false; info.nota = 'devuelve HTML'; }
        }
        resultado.EMPLEADOS.rutas_probadas[ruta] = info;
      } catch (e) {
        resultado.EMPLEADOS.rutas_probadas[ruta] = { error: e.message };
      }
    }
