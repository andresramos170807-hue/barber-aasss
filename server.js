...
async function fetchAllPages(url, headers, maxPages = 15) {
  let results = [];
  let nextUrl = url;
  let pages = 0;
  let count = null;
...
async function getKoiboxData(local, question) {
  ...
  try {
    // Citas del mes (todas las páginas)
    const citasMesData = await fetchAllPages(
    ...
    const citasDiaData = await fetchAllPages(
      `https://api.koibox.cloud/api/agenda/?fecha__gte=${fechaEspecifica}&fecha__lte=${fechaEspecifica}&page_size=200`,
      headers, 3
    );

    // Ventas del mes (facturación real)
    const ventasData = await fetchAllPages(
    ...
    // Clientes
    const clientesData = await fetchAllPages(
      `https://api.koibox.cloud/api/clientes/?page_size=200`, headers, 3
    );
    ...
    const ticketMedio = totalCitasMes > 0 ? Math.round((facturacionTotal / totalCitasMes)*100)/100 : 0;
