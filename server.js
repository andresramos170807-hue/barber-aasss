...
// ---------- CALIDAD (viene de un formulario -> Google Sheets en CSV) ----------
const CALIDAD_CSV_URL = process.env.CALIDAD_CSV_URL || '';
...
async function getCalidad() {
  if (!CALIDAD_CSV_URL) return {};
  if (calidadCache.data && Date.now() - calidadCache.time < 15 * 60 * 1000) return calidadCache.data;

  try {
    const r = await fetch(CALIDAD_CSV_URL);
    if (!r.ok) return calidadCache.data || {};
    const filas = parseCSV(await r.text());
    if (filas.length < 2) return {};

    const cab = filas[0].map(normaliza);
    const buscar = (...claves) => cab.findIndex(h => claves.some(k => h.includes(k)));

    const iFecha = buscar('marca temporal', 'timestamp', 'fecha');
    const iLocal = buscar('local', 'salon', 'centro', 'barberia');
    const iEst   = buscar('estandarizada', 'estandar', 'calidad del servicio', 'servicio');
    const iPer   = buscar('personal', 'equipo', 'plantilla');
    const iObs   = buscar('observacion', 'comentario', 'nota');

    const porLocal = {};
    for (let i = 1; i < filas.length; i++) {
      const f = filas[i];
      const localBruto = normaliza(f[iLocal]);
      if (!localBruto) continue;

      // Emparejar con nuestros nombres de local
      let local = null;
      if (localBruto.includes('sevilla')) local = 'Sevilla Este';
      else if (localBruto.includes('bormujo')) local = 'Bormujos';
      else if (localBruto.includes('gine')) local = 'Gines';
      if (!local) continue;

      const num = (v) => { const n = parseFloat(String(v).replace(',', '.')); return isNaN(n) ? null : n; };
      const registro = {
        fecha: iFecha >= 0 ? f[iFecha] : '',
        calidad_estandarizada: iEst >= 0 ? num(f[iEst]) : null,
        calidad_personal: iPer >= 0 ? num(f[iPer]) : null,
        observaciones: iObs >= 0 ? f[iObs] : '',
      };

      // Nos quedamos con el mas reciente de cada local (las filas nuevas van abajo)
      porLocal[local] = registro;
    }

    calidadCache = { data: porLocal, time: Date.now() };
    return porLocal;
  } catch (e) {
    console.log('Error leyendo calidad:', e.message);
    return calidadCache.data || {};
  }
}
...
function calcularOKR(d, calidad) {
  ...
  return {
    ...
    calidad_estandarizada: {
      ...
      ultima_revision: cal.fecha || null,
    },
    calidad_personal: {
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && req.url.startsWith('/api/debug')) {
    ...
    const vd = await fetchAllPages(`https://api.koibox.cloud/api/ventas/?fecha__gte=${firstDay}&fecha__lte=${lastDay}&limit=${LIMITE}`, headers, 6);
