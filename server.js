...
function calcularRentabilidad(datos, cfg) {
  ...
  nombres.forEach((nombreConf) => {
    ...
    const brutoMes = bruto * (pagas / 12);
    const pctSS = (parseFloat(dc.porcentaje_seguridad_social) || 0) / 100;
    const ssMes = brutoMes * pctSS;
    const costeContrato = brutoMes + ssMes;
  ...
  return {
    _como_se_calcula: 'Los COSTES son los reales del mes completo (sueldos y gastos fijos enteros). Se comparan con la facturacion proyectada a fin de mes, para que la comparacion sea justa. Tambien se indica lo facturado hasta hoy.',
    incluye_iva: conIvaModo,
...
async function getDatosLocal(local, fechaEspecifica, mesPedido) {
  ...
  const ck2 = ck;
  const c = cacheGet(ck2);
  if (c) { console.log('CACHE', ck2); return c; }
