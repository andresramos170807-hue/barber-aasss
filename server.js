...
async function descargarDatosLocal(local, fechaEspecifica, mesPedido, ck, key) {
  ...
  // La facturacion del empleado = SOLO sus servicios (no los productos)
  const facturacionTotalEmp = Object.values(porEmpleado).reduce((a, d) => a + d.fact_servicios, 0) || 1;

  const rankingEmpleados = Object.entries(porEmpleado)
    .map(([nombre, d]) => {
      const fact = Math.round(d.fact_servicios * 100) / 100;
      ...
      return {
        nombre,
        facturacion: fact,
        facturacion_sin_iva: Math.round(d.base_servicios * 100) / 100,
        facturacion_con_productos: Math.round(d.facturacion * 100) / 100,
        tickets: d.tickets,
        ticket_medio: d.tickets > 0 ? Math.round((fact / d.tickets) * 100) / 100 : 0,
        porcentaje_del_total: Math.round((fact / facturacionTotalEmp) * 1000) / 10,
        ...
        media_diaria: day > 0 ? Math.round((fact / day) * 100) / 100 : 0,
      };
    })
...
const server = http.createServer(async (req, res) => {
  ...
  if ((req.url.startsWith('/api/debug') || req.url.startsWith('/api/probargastos') || req.url.startsWith('/api/probarfotos')) && usuario.rol !== 'admin') {
