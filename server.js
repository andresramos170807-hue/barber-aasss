...
async function leerGastosSalon(local, mesPedido) {
  ...
  try {
    ...
    for (let i = filaCab + 1; i < filas.length; i++) {
      ...
      if (normaliza(concepto).includes('total gastos')) {
    ...
    return { conceptos: conceptosFinal, total: totalFinal, total_excel: totalExcel };
  } catch (e) {
...
async function getKoiboxData(local, question, mesPedido) {
  ...
  d.rentabilidad = calcularRentabilidad(d, await getCostes(mesPedido));
  return d;
}
